// carrie-chat.js — RESTORED AUTO-GREET + WORKING MODE SWITCHES + SUPABASE MONITORING
...
(function () {
  ...
  // Initialize Supabase (only if credentials are added)
  function initSupabase() {
    if (SUPABASE_URL === 'YOUR_SUPABASE_URL_HERE' || SUPABASE_ANON_KEY === 'YOUR_ANON_KEY_HERE') {
      console.log('ℹ️ Supabase not configured. Chat works normally without monitoring.');
      return false;
    }
    
    try {
      supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
      console.log('✅ Supabase connected - messages will be saved for monitoring!');
      return true;
    } catch(e) {
  ...
  // Save message to Supabase (only if configured and not in anonymous mode)
  async function saveMessageToSupabase(role, text) {
    if (!supabase) return; // Supabase not configured - skip silently
    
    // Check anonymous mode (owner only)
    const isAnonymous = localStorage.getItem("anonymousMode") === "true";
    if (isAnonymous && isOwner()) {
      console.log("👻 Anonymous mode: Not saving to Supabase");
      return;
    }
    
    const userId = await getCurrentUserId();
    if (!userId) {
      console.log('⚠️ No user ID, cannot save to Supabase');
      return;
    }
  ...
  function addMessage(role, text, messageId = null) {
    ...
    // SAVE TO SUPABASE (if configured and not in anonymous mode)
    saveMessageToSupabase(role, text);
