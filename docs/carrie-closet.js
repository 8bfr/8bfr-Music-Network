...
(function () {
  ...
  function canUse(storage) {
    try { const k="__t"; storage.setItem(k,"1"); storage.removeItem(k); return true; } catch(e){ return false; }
  }
  const hasLS = canUse(window.localStorage);

  function cookieSet(val){ try{ document.cookie=encodeURIComponent(STORE_KEY)+"="+encodeURIComponent(val)+"; path=/; max-age=31536000; SameSite=Lax"; return true;}catch(e){return false;} }
  function cookieGet(){ try{ const key=encodeURIComponent(STORE_KEY)+"="; const parts=(document.cookie||"").split("; "); for(const p of parts){ if(p.indexOf(key)===0) return decodeURIComponent(p.slice(key.length)); } return null;}catch(e){return null;} }
  function nameGet(){ try{ const raw=window.name||""; if(!raw) return null; const data=JSON.parse(raw); if(!data||typeof data!=="object") return null; return data[STORE_KEY]||null;}catch(e){return null;} }
  function nameSet(val){ try{ const raw=window.name||""; let data={}; if(raw){ const parsed=JSON.parse(raw); if(parsed&&typeof parsed==="object") data=parsed; } data[STORE_KEY]=val; window.name=JSON.stringify(data); return true;}catch(e){return false;} }
  ...
  function loadState(itemsMaybe){
    try{
      ...
      if(data.equippedByGender||data.equippedFallbackByGender){
        restoreSet("female", data.equippedByGender&&data.equippedByGender.female, data.equippedFallbackByGender&&data.equippedFallbackByGender.female);
        restoreSet("male", data.equippedByGender&&data.equippedByGender.male, data.equippedFallbackByGender&&data.equippedFallbackByGender.male);
      }else{
  ...
  function setBaseImage(){ const baseImg=$("#closetBaseImg"); if(!baseImg) return; const g=currentGender, s=currentSkin; baseImg.src=(g==="female"?`assets/images/base/female/base_female_${s}.png?v=15`:`assets/images/base/male/base_male_${s}.png?v=15`); }
  function updateLabels(){ const gLabel=currentGender==="female"?"Female":"Male", sLabel=currentSkin==="light"?"Light skin":"Dark skin"; if(previewLabel) previewLabel.textContent=`${gLabel} • ${sLabel} • Bikini base`; if(closetGenderLabel) closetGenderLabel.innerHTML=`Showing items for <b>${gLabel}</b> avatar`; }
  ...
  function setGender(newGender){ saveState(); currentGender=newGender; document.body.dataset.gender=currentGender; equipped=equippedSets[currentGender]||equippedSets.female; setBaseImage(); updateLabels(); renderItems(); renderOverlays(); saveState(); }
  function pickImgForSkin(itemObj){ return (currentSkin==="dark"&&itemObj.imgDark)?itemObj.img:itemObj.img; }

  function cardForItem(itemObj){
    const card=document.createElement("div"); card.className="closet-item-card";
    const thumbWrap=document.createElement("div"); thumbWrap.className="closet-item-thumb";
    const t=document.createElement("img"); t.src=itemObj.thumb||pickImgForSkin(itemObj); t.alt=itemObj.name||itemObj.id; thumbWrap.appendChild(t);
    const meta=document.createElement("div"); meta.className="min-w-0";
    const title=document.createElement("div"); title.className="text-[12px] font-semibold truncate"; title.textContent=itemObj.name||itemObj.id;
    const sub=document.createElement("div"); sub.className="text-[10px] text-purple-200/70 truncate"; sub.textContent=itemObj.label||itemObj.category;
    meta.appendChild(title); meta.appendChild(sub); card.appendChild(thumbWrap); card.appendChild(meta);

    const slot=itemObj.slot; const isOn=equipped[slot]&&equipped[slot].id===itemObj.id;
  ...
  function filterItems(items){ return items.filter(it=>{ const gOK=it.gender==="unisex"||it.gender===currentGender; if(!gOK) return false; if(currentCat==="all") return true; return (it.category===currentCat||it.cat===currentCat); }); }
  function renderItems(){ const items=safeItems(); if(!items||!grid) return; const list=filterItems(items); grid.innerHTML=""; if(list.length===0){ emptyMsg&&emptyMsg.classList.remove("hidden"); return;} else{ emptyMsg&&emptyMsg.classList.add("hidden"); } list.forEach(it=>grid.appendChild(cardForItem(it))); }
  ...
  function addOverlayImg(itemObj){
    ...
    // EARRINGS (2 pieces)
    if(slot==="ears"){
      const offset=itemObj.offsetX||20;
      ["left","right"].forEach((side,i)=>{
        const img=document.createElement("img");
        img.src=src; img.alt=itemObj.name||itemObj.id;
        img.className=`layer-overlay item-${itemObj.id} layer-ears-${side} layer-${side}`;
        img.style.zIndex=String(zBySlot.ears||50);
        img.style.position="absolute";
        img.style.left=`calc(50% ${side==="left"? "-":"+"} ${offset}px)`;
    ...
    if(slot==="shoes"){
      ...
      ["left","right"].forEach((side,i)=>{
        const img=document.createElement("img");
        img.src=src; img.alt=itemObj.name||itemObj.id;
        img.className=`layer-overlay item-${itemObj.id} layer-shoes-${side} layer-${side}`;
    ...
    const img=document.createElement("img");
    img.src=src; img.alt=itemObj.name||itemObj.id; img.className=`layer-overlay item-${itemObj.id}`;
  ...
  function syncUIButtons(){$$(".seg-btn[data-gender]").forEach(b=>b.classList.toggle("active",b.dataset.gender===currentGender)); $$(".tab-btn").forEach(b=>b.classList.toggle("active",b.dataset.cat===currentCat)); }
  ...
  function bootWhenReady(){
    ...
    if(itemsNow){
      ...
      document.body.dataset.skin=currentSkin;
      initTabs(); initGenderButtons(); buildSkinButtons(); syncUIButtons(); setBaseImage(); updateLabels(); renderItems(); renderOverlays();
      return;
    }
    let tries=0;
