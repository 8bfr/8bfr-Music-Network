...
// For now, James & Azreen reuse Carrie's videos so they always show.
const AVATAR_VIDEOS = {
  carrie: {
...
function loadOwnerSimulate() {
  ...
  try {
    const val = localStorage.getItem("carrie_owner_view");
    ownerSimulateUser = val === "user";
  } catch (e) {
    ownerSimulateUser = false;
  }
}
...
// In future, check Pro membership. For now:
// - Owner is always Pro
// - Everyone else = not Pro
function isProActive() {
  return effectiveRole() === "owner";
}

// avatar helper (for chat bubbles)
...
function wireShopMenu() {
  // No JS needed here since we used <a href> links.
  // Kept function so nothing breaks if we want custom behavior later.
}
